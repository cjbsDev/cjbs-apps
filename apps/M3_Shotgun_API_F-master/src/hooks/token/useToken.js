...
export function getToken(req = null, res = null) {
  ...
  try {
    const encryptAccessToken = req ? getCookie('saasHYMSSW', {req, res}) : getCookie('saasHYMSSW');
...
export function getRefreshToken(req = null, res = null) {
  ...
  try {
    const encryptToken = req ? getCookie('saasIT', {req, res}) : getCookie('saasIT');
...
export function setToken(tokenInfo, req, res) {
  ...
  const options = req
    ? {
        ...
        // domain: process.env.NODE_ENV === 'production' ? 'cjbioscience.com' : 'localhost',
        domain: process.env.NODE_ENV === 'production' ? 'ezbiocloud.net' : 'localhost',
      }
    : {
        expires: rfExpiresDate,
        secure: process.env.NODE_ENV === 'production',
  ...
  deleteCookie('saasHYMSSW', {req, res});
  setCookie('saasHYMSSW', encryptAccessToken, options);

  deleteCookie('saasIT', {req, res});
  setCookie('saasIT', encryptRefreshToken, options);
}
